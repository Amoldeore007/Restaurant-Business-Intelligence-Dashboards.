...
3. **Data Modeling (Star Schema):** 
   * Built centralized **Fact Tables** for daily transactional metrics and order volumes
   * Connected relevant dimension tables to establish robust relationship models, ensuring optimal filter propagation.
4. **Calculations & DAX:** Added calculated columns and advanced measures (e.g., Average Order Value, Net Sales Growth %, Settlement Rate, and Outstanding Amounts) to power executive KPIs

---

##  Dashboard Views & Key Insights
The report is structured into insights across multiple analytical dimensions:
* **Executive Sales Overview:** Tracks total net sales (~₹30.75M), total orders (6,481), Average Order Value (~₹493.56), and monthly sales growth trends
* **Sales & Category Analysis:** Compares ordered vs. settled amounts, highlighting high-performing categories like Main Course and Breads, along with payment method breakdowns (Cash, UPI, Card)
* **Order Channel & Operations:** Analyzes hourly order distributions to identify peak dining rush hours (notably between 7 PM and 9 PM) for better staffing and kitchen prep
* **Product Performance Analysis:** Evaluates top-selling items by quantity and revenue (such as Mineral Water, Kesar Rasmalai, and Aloo Paratha) alongside individual item unit pricing
